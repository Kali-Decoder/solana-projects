use crate::{error::ErrorCode, state::escrow::Escrow};
...
pub fn _initialize_exchange(
    ...
    if(a_to_b_amount<=0 || b_to_a_amount<=0){
        return Err(ErrorCode::NotValidAmount.into());
    }
    ...
    transfer(transfer_context, a_to_b_amount)?;
    Ok(())
}
